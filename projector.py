...
def project(
    ...
    if loss_paper in ['sgan2', 'im2sgan', 'discriminator']:
        ...
        vessel_gt = vesselseg(tmp_gt_img).detach() ## unetseg input should be (0~1)
        print(vessel_gt.size())
        
        out = torch.sigmoid(vessel_gt)  ### value 0-1.0
        parsing = out.squeeze().cpu().numpy()
    ...
    for step in range(num_steps):
        ...
        ###################### vessel seg #########################################
        tmp_gen_img = synth_images.detach() ### (-1.0~1.0)
        ...
        vessel_gen = vesselseg(tmp_gen_img).detach() ## unetseg input should be (0~1)
        ...
        if loss_paper == 'sgan2':
            ...
            seg_loss = 0.1*dicebce_loss(vessel_gen,vessel_gt)
            loss = dist + reg_loss * regularize_noise_weight + seg_loss
            # Print in the same line (avoid cluttering the commandline)
            n_digits = int(np.log10(num_steps)) + 1 if num_steps > 0 else 1
            message = f'step {step + 1:{n_digits}d}/{num_steps}: dist {dist:.7e} | loss {loss.item():.7e} | seg {seg_loss.item():.7e}'
            print(message, end='\r')
            ...
            loss = percept_error + mse_error   # + 1e-2 * (1 - ssim_loss)  # needs to be fine-tuned
            # loss = percept_error + mse_error +seg_loss + 1e-2 * (1 - ssim_loss)
            # print(percept_error,mse_error,seg_loss)
            # Noise regularization.
            reg_loss = 0.0
            for v in noise_buffs.values():
                noise = v[None, None, :, :]  # must be [1,1,H,W] for F.avg_pool2d()
            ...
            n_digits = int(np.log10(num_steps)) + 1 if num_steps > 0 else 1
            message = f'step {step + 1:{n_digits}d}/{num_steps}: percept loss {percept_error.item():.7e} | ' \
                      f'pixel mse {mse_error.item():.7e} | ssim {ssim_loss.item():.7e} | loss {loss.item():.7e | segloss {seg_loss.item()}:.7e}'
            print(message, end='\r')
